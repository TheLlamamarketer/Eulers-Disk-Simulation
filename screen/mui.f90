...
!
         ...
         if (ierr /= 0) then
            ...
            range_eps = 1.0e-12_8
            scale = 1.0_8
            if (present(minv)) scale = max(scale, abs(minv))
            if (present(maxv)) scale = max(scale, abs(maxv))
            range_eps = range_eps*scale
            if (present(minv)) then
               if (dval < minv - range_eps) then
                  call errmsg( ERR_UNDERFLOW)
                  ierr = -1
               else if (dval < minv) then
                  dval = minv
               endif
            endif
            if (present(maxv)) then
               if (dval > maxv + range_eps) then
                  call errmsg( ERR_OVERFLOW)
                  ierr = -1
               else if (dval > maxv) then
                  dval = maxv
               endif
            endif
         endif
